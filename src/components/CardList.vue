  ...
  <section :style="`background: ${options.color}`" @drop="onDrop($event, options.id)" @dragover.prevent
    ...
    <v-menu>
      <template v-slot:activator="{ props }">
        <v-btn class="my-4" color="primary" v-bind="props">
          Сортировка по Рейтингу
        </v-btn>
      </template>
      <v-list>
        <v-list-item>
          <v-list-item-title @click="sorting('rising')">по возрастанию</v-list-item-title>
          <v-list-item-title @click="sorting('descending')">по убыванию</v-list-item-title>
          <v-list-item-title @click="sorting('notsorted')">без сортировки</v-list-item-title>
        </v-list-item>
      </v-list>
    </v-menu>
...
function sorting(action) {
  getLocalCards();
  action == 'rising' ? cards = cards.value.sort((a, b) => a.rating.rate - b.rating.rate) : false
  action == 'descending' ? cards = cards.value.sort((a, b) => b.rating.rate - a.rating.rate) : false
  action == 'notsorted' ? cards = cards.value.sort((a, b) => a.id - b.id) : false
}
///
</script>
